...
/-! # `FallbackReceipt` proofs — action `deliver_entry_fastqc`

Scaffolded by `#gen_proof_files FallbackReceipt`; yours to edit. Proves every
registered VC of `deliver_entry_fastqc` cross-file from the module's persisted VC registry
(`veil.gen.vcRegistry`), persists them as kernel-checked theorems in this
file's olean, and emits the per-action preservation lemma consumed by
`Certify.lean`'s `#gen_composition`.

Manual cells go on `#prove_vc FallbackReceipt deliver_entry_fastqc <property> by <tac>` lines
...
#prove_action FallbackReceipt deliver_entry_fastqc
