...
/-! # `FallbackReceipt` proofs — action `build_entry_equiv`

Scaffolded by `#gen_proof_files FallbackReceipt`; yours to edit. Proves every
registered VC of `build_entry_equiv` cross-file from the module's persisted VC registry
(`veil.gen.vcRegistry`), persists them as kernel-checked theorems in this
file's olean, and emits the per-action preservation lemma consumed by
`Certify.lean`'s `#gen_composition`.

Manual cells go on `#prove_vc FallbackReceipt build_entry_equiv <property> by <tac>` lines
...
#prove_action FallbackReceipt build_entry_equiv
