...
/-! # `Mvba` proofs — action `leader_repropose`

Scaffolded by `#gen_proof_files Mvba`; yours to edit. Proves every
registered VC of `leader_repropose` cross-file from the module's persisted VC registry
(`veil.gen.vcRegistry`), persists them as kernel-checked theorems in this
file's olean, and emits the per-action preservation lemma consumed by
`Certify.lean`'s `#gen_composition`.

Manual cells go on `#prove_vc Mvba leader_repropose <property> by <tac>` lines
...
#prove_action Mvba leader_repropose
