...
/-! # `Chorus` proofs — action `redisseminate_chunk`

Scaffolded by `#gen_proof_files Chorus`; yours to edit. Proves every
registered VC of `redisseminate_chunk` cross-file from the module's persisted VC registry
(`veil.gen.vcRegistry`), persists them as kernel-checked theorems in this
file's olean, and emits the per-action preservation lemma consumed by
`Certify.lean`'s `#gen_composition`.

Manual cells go on `#prove_vc Chorus redisseminate_chunk <property> by <tac>` lines
...
#prove_action Chorus redisseminate_chunk
