...
recorded backing invariant). Stubs generated by Veil's "insert theorem
stubs" suggestion; pre-M6 these were `@[veil]` interactive dischargers
for the in-file sweep — now preproven cells the `#prove_action` below
consumes as-is after a statement check. The `hinv.2.…`
