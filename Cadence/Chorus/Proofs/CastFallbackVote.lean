...
/-! # `Chorus` proofs — action `cast_fallback_vote`

Scaffolded by `#gen_proof_files Chorus`; yours to edit. Proves every
registered VC of `cast_fallback_vote` cross-file from the module's persisted VC registry
(`veil.gen.vcRegistry`), persists them as kernel-checked theorems in this
file's olean, and emits the per-action preservation lemma consumed by
`Certify.lean`'s `#gen_composition`.

Manual cells go on `#prove_vc Chorus cast_fallback_vote <property> by <tac>` lines
...
#prove_action Chorus cast_fallback_vote
