...
/-! # `Chorus` proofs — action `byz_sign_fallback`

Scaffolded by `#gen_proof_files Chorus`; yours to edit. Proves every
registered VC of `byz_sign_fallback` cross-file from the module's persisted VC registry
(`veil.gen.vcRegistry`), persists them as kernel-checked theorems in this
file's olean, and emits the per-action preservation lemma consumed by
`Certify.lean`'s `#gen_composition`.

Manual cells go on `#prove_vc Chorus byz_sign_fallback <property> by <tac>` lines
...
#prove_action Chorus byz_sign_fallback
