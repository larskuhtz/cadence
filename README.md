...
path costs ~7.6 KB across the object list. **Fix: check the repository out at a
shorter path** (about 35 characters or fewer works on macOS 15/arm64), or build
on Linux, where the limit is far higher. Nothing else needs to change; once
...
scripts/                           staged build + monitor runners and regression suites
