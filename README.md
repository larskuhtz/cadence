...
| **Speculative-finality revertibility** ("reverted only under equivocation") | `Cadence/Chorus.lean` (`speculative_agreement_*`) | reconstructed VCs, conditional on `no_equivocation`; scope caveat — the paper's invalid-encoding culprit case is abstracted away, see [docs/ChorusDesign.md](./docs/ChorusDesign.md) §3.4 |
