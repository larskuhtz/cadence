...
| `Cadence/Cadence.lean` | the pipelining glue: 6 actions (two oracle steps, two handlers, two protocol actions), 4 safety + 20 invariants, against `OrchestratorSafety`/`SlotConsensusSafety` as class constraints | 175 VCs ✅ + 2 `sat trace` ✅ + `#gen_theorems`, at **`veil.smt.trust false`** (since 2026-07-07; contract-constraint form since 2026-09-04) | ~60 s |
| `Cadence/Conductor.lean` | the orchestrator: 7 actions, 5 safety + 15 invariants, against `ACSSafety` as a class constraint | 168 VCs ✅ + 2 `sat trace` ✅ + `#gen_theorems`, at **`veil.smt.trust false`** (since 2026-07-10; contract-constraint form since 2026-09-04) | ~60 s |
| `Cadence/Composition.lean` | `invariants_of_reachable` + named `reachable_<property>` projections for both small models (emitted by `#gen_composition` since 2026-09-09), `Conductor ⊨ OrchestratorSafety` (every field, the two-state ones from the transition bodies), `Conductor.OrchestratorResidual` + `orchestrator_of_residual`, positional MCP Safety (`positional_log_safety`) | plain Lean over the persisted VC theorems and Veil's transition definitions — kernel-checked; axioms `#guard_msgs`-pinned to the three-axiom form | ~10 s |
| `Cadence/System.lean` | the composed system: the glue's MCP Safety at the Conductor and Chorus instances (`system_positional_log_safety`) | plain Lean; axiom-pinned | ~5 s |
| `Cadence/Chorus.lean` | per-slot consensus: 38 actions, 9 safety + 88 invariants | **model file** — elaborates the transition system and persists the VC registry (two entries per cell, plus the initializer's); runs no sweep | ~2 min |
| `Cadence/Chorus/Proofs/` ×39 | one file per action (+ `Init`): `#prove_action` re-creates that action's registered VC statements and persists fresh kernel-checked reconstructions as **real proofs**, plus the per-action preservation lemma. The manual quorum-intersection cells are `#prove_vc … by <tactic>` cells in these files, consumed after a statement check | a fresh kernel-checked re-proof for each registered cell, the manual ones included; retry ladder + alternative-encoding fallback built in | warm ~20–30 s per 6-file batch; cold minutes per file |
| `Cadence/Chorus/Certify.lean` | `#gen_composition`: `Chorus.invariants_of_reachable` (39 cases over the preservation lemmas) + `Chorus.reachable_<property>` ×97 | kernel-checked at emission; three-axiom `#guard_msgs` pin **and** the pinned `#veil_status Chorus` audit (every registry cell real; the number lives in the pin itself) | ~40 s (dominated by the audit walk) |
| `Cadence/Chorus/Compose.lean` | `Chorus.slotConsensusSafety` — the slot-indexed family of Chorus copies as the state-level slot-consensus contract (tagged decision vectors; agreement / slot-safety / proposal-inclusion; the two-state fields from all 38 actions' transition bodies), plus `Chorus.SlotConsensusResidual` and `slotConsensus_of_residual` | plain Lean; axiom-pinned to the three-axiom form | ~25 s |
...
| `Cadence/Mvba/Compose.lean` | `Mvba.mvbaSafety` — Mvba ⊨ `MVBASafety` (every field; the two-state fields from all 24 actions' transition bodies), `Mvba.MvbaResidual` (the clock, the admissible-run model, `ℓ`, Termination — nothing safety-shaped) and `mvba_of_residual`, which also discharges the inputs, their observables, effects, frames and Quiescence | plain Lean; axiom-pinned | ~1 min |
| `Cadence/Mvba/NoLock.lean` | the MVBA instantiation with the `Pre-Prepare` handler's lock check removed, on a restriction of the mutant (bulk `propose`/availability/Byzantine steps, a theory-fixed scheduler and adversary) every run of which is a run of the mutant | `#model_check` ❌ violation of `agreement` — a 28-step trace with commit certificates on both values and two correct validators deciding differently, `#guard_msgs`-pinned (**the violation is the expected result**) | ~2½ min |
