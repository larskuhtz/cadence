...
| `Cadence/Chorus/Proofs/` ×39 | one file per action (+ `Init`): `#prove_action` re-creates that action's registered VC statements and persists fresh kernel-checked reconstructions as **real proofs**, plus the per-action preservation lemma. The 11 manual quorum-intersection cells are `#prove_vc … by <tactic>` cells in these files, consumed after a statement check | 3 811 re-proofs (3 822 cells − 11 manual); retry ladder + alternative-encoding fallback built in | warm ~20–30 s per 6-file batch; cold minutes per file |
