...
`LEAN_NUM_THREADS=4`). Thirteen files needed mechanical repair, in four categories: `simp`/`dsimp`/`simpa` calls that unfold a class-valued definition now need `+instances` (Lean 4.32 defaults `Simp.Config.instances` to `false`); eleven `def`s whose type is a class need `@[implicit_reducible]`, and in the monitor the missing attribute broke instance *synthesis*, not just the lint; `Chorus.lean` needs `maxRecDepth` raised **before** its action declarations, because the new frontend nests one `openStateAround` per statement; and one `acs_decide` call site in `Cadence/Composition.lean` lost a positional `_`, since Veil now deduplicates identical `Decidable` side conditions in a generated theorem's telescope. Two dependency simplifications came with it: the `larskuhtz/loom` fork was dropped (it existed only because the previous fork set `precompileModules`, which this one does not), and with no `:shared` target forced on Mathlib the macOS argument-limit link failure is gone — a native build now works on macOS from any checkout path, so every audit tier is reachable without a container. **The measured cost of that simplification is a ~3× slower warm path**: a full warm re-validation (project oleans deleted, cache kept) runs 10 min 54 s at the default `BATCH=6` against 3 min 35 s before, and a Chorus proof batch of six 48–55 s against 18–28 s, on the same machine and with an identical workload (843 ✅ / 16 324 ♻). The dominant term is proof-file replay, which is Veil meta-code now running interpreted rather than precompiled; model elaboration under the new frontend contributes only ~1.35×. Restoring the flag was tried and does not work on these pins — Loom's overlapping library globs break `CaseStudies:shared`, and a Veil built against a successfully linked Mathlib shared library then segfaults on load. Peak resident memory improved, 18.8 GB → 15.3 GB. |
