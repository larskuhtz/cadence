...
* **`{model M "safety [x]"}`** quotes a declaration of a Veil model from the
  ...
  from the JSON that `scripts/docs.sh` stage 2 writes. Veil's `safety`,
  `action` and `relation` are not Lean declarations, so this is the only way
  to put the real text of a model on the page; with `proven := L` it adds the
...
Every link from the guide and from the trust boundary into the sources is
computed from the renderer's naming, and `scripts/docs.sh` checks each one
against the rendered pages. A declaration generated by a Veil command (the
`reachable_*` projections, `invariants_of_reachable`) has no syntax of its own
and so no anchor on its page; it links to the page and is marked as generated.
