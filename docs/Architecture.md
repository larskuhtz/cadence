...
cold). The 14 quorum-intersection cells that SMT cannot find are plain
manual theorems in their actions' proof files, consumed after a
statement check. `<Model>/Certify.lean` composes the per-action lemmas
